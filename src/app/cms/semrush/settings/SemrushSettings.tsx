...
import { semrushApi } from '@/lib/semrush-api';

export default function SemrushSettings() {
  ...
  const handleTestApi = async () => {
    ...
    setTestResult(null);
    try {
      const result = await semrushApi.testApi(apiKey);
        ...
        <div className="space-y-6">
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-gray-400 mb-2">Semrush API Key</label>
                ...
                {testResult && (
                  <p
                    className={`mt-2 text-sm ${testResult.startsWith('✓') ? 'text-green-400' : 'text-red-400'}`}
                  >
                    {testResult}
                  </p>
                )}
          ...
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-2">Default Domain</label>
              <input
                type="text"
                value={domain}
