#!/usr/bin/env node
/**
 * Push docs from .linear-doc-cache/payloads → Linear project documents.
 * Requires LINEAR_API_KEY. Used by sync:linear; MCP path uses save_document directly.
 */
...
const PAYLOAD_DIR = join(__dirname, '.linear-doc-cache/payloads');
...
async function listProjectDocs() {
  const data = await gql(
    `query($id: String!) { project(id: $id) { documents { nodes { id title content url slug } } } }`,
    { id: PROJECT_ID }
  );
...
async function main() {
  ...
  for (const f of files) {
    const p = JSON.parse(readFileSync(join(PAYLOAD_DIR, f), 'utf8'));
    ...
    if (hit?.id) {
      const data = await gql(
        `mutation($id: String!, $input: DocumentUpdateInput!) {
          documentUpdate(id: $id, input: $input) { document { id slug title url } }
        }`,
        { id: hit.id, input: { title: p.title, content: p.content } }
      );
      const doc = data.documentUpdate.document;
      state.documents[repoPath] = { id: doc.id, slug: doc.slug, hash, title: doc.title, url: doc.url };
      updated++;
      console.log('update', repoPath);
    } else {
      const data = await gql(
        `mutation($input: DocumentCreateInput!) {
          documentCreate(input: $input) { document { id slug title url } }
        }`,
